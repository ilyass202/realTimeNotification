...
public class TokenManager {
    ...
    public ResponseRegToken saveToken(RegisterToken register){
        ReferentielUser user = userRepo.findById(register.userId()).orElseThrow(()-> new EntityNotFoundException("le user nest pas trouvé"));
    ...
    public ResponseToken getToken(TokenRequest token){
         ReferentielUser user = userRepo.findById(token.userId()).orElseThrow(()-> new EntityNotFoundException("le user nest pas trouvé"));
